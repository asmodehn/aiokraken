...
class AssetPairs:
    ...
    async def retrieve(cls, pairs: typing.List[str] = None, rest: RestClient = None, ):
        ...
        cls._assetpairs = await rest.retrieve_assetpairs(pairs=pairs)
        return cls._assetpairs

    _assetpairs: typing.ClassVar[AssetPairsMapping]

    _proxy: MappingProxyType

    _ohlcvs: typing.Dict[KTimeFrameModel, OHLC]

    def __new__(cls, pairs = None, rest: RestClient=None, ):
        if not hasattr(cls, "_assetpairs"):
            # on first instance, trigger the classwide retrieval
            asyncio.run(cls.retrieve(rest=rest))

        return super(AssetPairs, cls).__new__(cls)

    def __init__(self, pairs = None, rest: RestClient=None,):
        self._proxy = MappingProxyType({n: d for n, d in self._assetpairs.items() if
                                        n in pairs or
                                        d.altname in pairs or
                                        d.wsname in pairs})
    ...
    async def ticker(self, rest: RestClient = None):
        ...
        t = await rest.ticker(pairs=self.keys())
    ...
    async def trades(self, rest: RestClient = None, start: datetime = None, stop: datetime = None):
    ...
    def ohlcv(self, rest: RestClient = None):  # TODO: , start: datetime = None, stop: datetime = None):
...
if __name__ == '__main__':
    ...
    XBTEUR_XTZEUR = AssetPairs(rest=rest, pairs=["XBT/EUR", "XTZ/EUR"])

    # now we are talking about XBT/EUR or XTZ/EUR...

    print(XBTEUR_XTZEUR)  # the assetpair/market data

    for o, od in XBTEUR_XTZEUR.ohlcv(rest=rest).items():
