...
class TickerFrame(TimeindexedDataframe):
    ...
    def __init__(self, dataframe: pd.DataFrame):
        ...
        super(TickerFrame, self).__init__(data=dataframe, tz=timezone.utc)

    def __repr__(self):
        # TODO : HOWTO do this ?
        return repr(self.dataframe)

    def stitch(self, th: TickerFrame) -> TickerFrame:
        ...
        new_th = TickerFrame(dataframe=newdf.copy())  # explicit copy to prevent modifying self.
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, slice):
            ...
            return TickerFrame(dataframe=self.dataframe[item].copy())
...
def tickerframe(tickers_as_dict: typing.Dict[str, Ticker]) -> Result[TickerFrame]:
    if not tickers_as_dict:
        df = pd.DataFrame(columns=[f.name for f in dataclasses.fields(Ticker)])  # we only know about index name
    else:
        # Note we drop the key (should already be replicated in the value)
        df = pd.DataFrame.from_records(data=[dataclasses.asdict(v) for v in tickers_as_dict.values()])

    return Ok(TickerFrame(dataframe=df))
